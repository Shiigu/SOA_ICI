...
#define RADIO_TERRESTRE 6371000
#define NOMBRE_RED "RED"
#define PASSWORD_RED "PASSWORDRED"
...
struct tm       time_stamp_final;
struct tm       time_stamp_inicial;
...
float           umbral_temperatura_minimo = 0;
...
float           color_distancia_S_inicio = 1;
float           color_distancia_V_inicio = 1;
int             color_distancia_H_fin = 357;
float           color_distancia_S_fin = 1;
float           color_distancia_V_fin = 1;
...
void setup()
{
  ...
  Conectar(NOMBRE_RED, PASSWORD_RED, "www.dweet.io");
...
void loop()
{
  ...
  while (true)
  {
    ...
    if (ciclos == 5)
    {
      ...
      if (time_stamp_inicial.tm_year == time_stamp_final.tm_year  &&
          time_stamp_inicial.tm_mon == time_stamp_final.tm_mon   &&
          time_stamp_inicial.tm_mday == time_stamp_final.tm_mday &&
          time_stamp_inicial.tm_hour == time_stamp_final.tm_hour &&
          time_stamp_inicial.tm_sec == time_stamp_final.tm_sec
         )
      {

        pantalla->setCursor(0, 3);
        pantalla->print("Conexion:");
        pantalla->printByte(1);
      }
      else
      {
        time_stamp_inicial.tm_year = time_stamp_final.tm_year;
        time_stamp_inicial.tm_mon = time_stamp_final.tm_mon;
        time_stamp_inicial.tm_mday = time_stamp_final.tm_mday;
        time_stamp_inicial.tm_hour = time_stamp_final.tm_hour;
        time_stamp_inicial.tm_min = time_stamp_final.tm_min;
        time_stamp_inicial.tm_sec = time_stamp_final.tm_sec;
        pantalla->setCursor(0, 3);
        pantalla->print("Conexion:");
...
void HumedadTemperatura()
{
  ...
  float lectura_temperatura = (((lectura_temperatura_g) * 100) / (umbral_temperatura_maximo+ umbral_temperatura_minimo)) / 100;
...
void Hora()
{
  ...
  uint8_t azul;
    ...
    if (cliente_tiempo->getHours() == 0)
      lectura_hora = ((((cliente_tiempo->getMinutes())) *60* 100.0) / 86400.0) / 100;
    else
      lectura_hora = ((((cliente_tiempo->getHours() * 60 + cliente_tiempo->getMinutes()))*60 * 100) / 86400.0) / 100;
...
void Distancia()
{
  ...
  if (ddistancia <= 300.0)
  {
    ...
    pantalla->print("m");
    distancia = ((ddistancia * 100) / (float) 300) / 100;
  ...
  {
    pantalla->setCursor(11, 3);
    pantalla->print("Estalejos");

    HSVaRGB(   color_distancia_H_fin,
               color_distancia_S_inicio,
               color_distancia_V_inicio,
               &rojo,
               &verde,
               &azul
           );
...
void ConsultarConfiguracion(char *host, char *destino)
{
  ...
  if (datos.length() > 10 && datos.indexOf("\"this\":\"failed\"") == -1)
  {
    ...
    umbral_temperatura_minimo = colores.substring(colores.indexOf("<Threshold>") + 11, colores.indexOf("</Threshold>")).toFloat();
    if (umbral_temperatura_minimo < 0)
      umbral_temperatura_minimo = -1 * umbral_temperatura_minimo;
    ...
    umbral_temperatura_maximo = colores.substring(colores.indexOf("<Threshold>") + 11, colores.indexOf("</Threshold>")).toFloat();
...
void ExtraerHorayFecha(String datos)
{
  String anio;
  String mes;
  String dia;
  ...
  anio = datos.substring(datos.indexOf("\"created\":\"") + 11, datos.indexOf("\"created\":\"") + 15);
  mes = datos.substring(datos.indexOf("\"created\":\"") + 16, datos.indexOf("\"created\":\"") + 18);
  dia = datos.substring( datos.indexOf("\"created\":\"") + 19, datos.indexOf("\"created\":\"") + 21);
  hora = datos.substring(  datos.indexOf("\"created\":\"") + 22, datos.indexOf("\"created\":\"") + 24);
  minutos = datos.substring(datos.indexOf("\"created\":\"") + 25, datos.indexOf("\"created\":\"") + 27);
  segundos = datos.substring(datos.indexOf("\"created\":\"") + 26, datos.indexOf("\"created\":\"") + 28);

  if (inicial == true)
  {
    time_stamp_inicial.tm_year = anio.toInt();
    time_stamp_inicial.tm_mon = mes.toInt();
    time_stamp_inicial.tm_mday = dia.toInt();
    time_stamp_inicial.tm_hour = hora.toInt();
    time_stamp_inicial.tm_min = minutos.toInt();
    time_stamp_inicial.tm_sec = segundos.toInt();
    time_stamp_final.tm_year = anio.toInt();
    time_stamp_final.tm_mon = mes.toInt();
    time_stamp_final.tm_mday = dia.toInt();
    time_stamp_final.tm_hour = hora.toInt();
    time_stamp_final.tm_min = minutos.toInt();
    time_stamp_final.tm_sec = segundos.toInt();
    inicial = false;
  }
  else
  {
